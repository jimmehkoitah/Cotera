...
def write_pq_jpeg(path, scene_nits, icc_bytes, quality=96, progressive=True):
    """8-bit PQ / BT.2020 JPEG carrying an ICC profile.

    JPEG is 8-bit, and PQ spreads 0..10000 nits across those 256 codes, so
    dithering is not optional here -- without it the panel gradient bands
    visibly. 4:4:4 chroma matters too: subsampling bleeds colour across the
    hard edge of a white mark on a saturated ground.

    Caveat worth repeating: ICC describes a colour space, it does not carry the
    "treat this as absolute luminance" signal that CICP does. A viewer that
    colour-manages this naively renders it far too dark rather than bright.
    """
    arr = scene_to_pq_rgb(scene_nits, bits=8, dither=True)
    Image.fromarray(arr, 'RGB').save(
        path, 'JPEG', quality=quality, subsampling=0, progressive=progressive,
        optimize=True, icc_profile=icc_bytes)
    return path
...
def write_sdr(path_png, path_jpg, scene_nits, sdr_white=k.SDR_WHITE_NITS,
    ...
    if path_png:
        im.save(path_png)
    if path_jpg:
        # Left deliberately untagged: an untagged JPEG is interpreted as sRGB,
        # which is what a control file should be. Embedding a profile here would
        # blur the very distinction the control exists to establish.
        im.save(path_jpg, quality=96, subsampling=0, optimize=True, progressive=True)
    return arr8
